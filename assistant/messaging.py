...
class MessageProcessor:
    ...
    def _validate_message_input(self, message: str, reasoning: str) -> None:
        ...
        if not isinstance(reasoning, str):
            raise ValueError(f"Reasoning must be a string, got {type(reasoning)}")
    ...
    def _validate_response_input(self, response: Any) -> None:
        ...
        if not hasattr(response, 'choices') or not isinstance(response.choices, list):
            raise ValueError("Response must have a 'choices' attribute that is a list")
        if not response.choices:
            raise ValueError("Response choices cannot be empty")
        if not hasattr(response.choices[0], 'message'):
            raise ValueError("Response choice must have a 'message' attribute")
        if not isinstance(response.choices[0].message, dict):  # Expecting a dictionary-like structure
            raise ValueError("Response message must be a dict")
        if 'content' not in response.choices[0].message or response.choices[0].message['content'] is None:
            raise ValueError("Response message must have a non-null 'content' attribute")
"""
Message processing and conversation flow management.
"""
import json
